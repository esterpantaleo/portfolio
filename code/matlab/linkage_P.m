...
%      'centroid' --- center of mass distance (approximate, computed
%                     using a formula that is exact if Y contains
%                     Euclidean distances)
...
if length(method) < 2
  error('The method given by the second argument is not defined.');
end 
...
if method == 'ce'  % square the Y so that it is easier to update.
  Y = Y .* Y;
elseif method == 'wa'
  Y = Y .* Y/2;
end 
...
for s = 1:(n-1)
  ...
  %%%
  case 'ha'
     A = sort(Z(s,1:2)); a = length(A);
     while A(a) > mm
     		  A = sort([A(1:a-1) Z(A(a)-mm,1:2)]);
           a = length(A);
     end
  %%%   
  ...
  if method == 'ha'
     for j = [1:i-1 i+1:m]
        if R(j)> mm
           B = sort(Z(R(j)-mm,1:2)); b = length(B);
           while B(b) > mm
              B = sort([B(1:b-1) Z(B(b)-mm,1:2)]);
              b = length(B);
           end
        else
           B = R(j);
